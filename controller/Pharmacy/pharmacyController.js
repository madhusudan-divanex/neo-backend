...
const patientHospitalAllotment = async (req, res) => {
    ...
    try {
        ...
        const isHospital = await User.findOne({
            _id: hospitalId,
            role: "hospital"
        });
        ...
        // total count
        const total = await BedAllotment.countDocuments({
            patientId,
            hospitalId
        });
        ...
        const allotments = await BedAllotment.find({
            ...
            .populate({
                path: "patientId",
                select: "name email unique_id",
                populate: {
                    path: "patientId",
                    select: "contactNumber profileImage"
                }
            })
            .populate("dischargeId", "createdAt")
            .populate("primaryDoctorId", "unique_id name")
        ...
        return res.status(200).json({
            success: true,
            message: "Patient allotment fetched",
            allotments,
        ...
        console.error("patientHospitalAllotment:", err);
...
const patientHospitalPrescriptions = async (req, res) => {
    const { patientId, hospitalId } = req.params;
    ...
    try {
        ...
        // total count
        const total = await Prescriptions.countDocuments({
            patientId,
            hospitalId
        });
        ...
        const prescriptions = await Prescriptions.find({
            ...
            .populate("allotmentId appointmentId", "customId")
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limit);
