...
const signUpDoctor = async (req, res) => {
    try {
        ...
        sendDoctorEmail("Email Template/doctor/VerifyOtp.html", { code, name: name || "Doctor" },
            "Verify Your Account", userData?._id)
        if (newDoctor) {
    ...
    try {
        ...
        if (withOtp) {
            ...
            if (email) {
                ...
                sendDoctorEmail("Email Template/doctor/VerifyOtp.html", { code, name: isExist?.name || "Doctor" },
...
const doctorKyc = async (req, res) => {
    ...
    try {
        ...
        if (data) {
            ...
            sendDoctorEmail("Email Template/doctor/Welcome.html",
                { btnLink: process.env.DOCTOR_URL + '/request-list', name: user?.name },
...
const getCustomProfile = async (req, res) => {
    ...
    try {
        let user;
        if (userId.length < 24) {
            user = await User.findOne({ nh12: userId, role: 'doctor' }).select('-passwordHash').lean();
        } else {
