...
const signUpLab = async (req, res) => {
    ...
    try {
        if (labId) {
            ...
            const isMax = await User.findOne({ email, _id: { $ne: isExist._id } })
            if (isMax) {
                return res.status(200).json({ message: "Email already exist", success: false })
            }
            const isContact = await User.findOne({ contactNumber, _id: { $ne: isExist?._id } })
            if (isContact) {
            ...
            if (newLab) {
                await User.findByIdAndUpdate(labId, { email, contactNumber, name, created_by_id, created_by: "self" }, { new: true })
            ...
            const isUser = await User.findOne({ email }) || await User.findOne({ contactNumber })
            if (isUser) {
            ...
            if (newLab) {
                const userData = await User.create({ name, contactNumber, email, role: 'lab', passwordHash: hashedPassword, labId: newLab?._id, created_by: 'self' })
...
const verifyOtp = async (req, res) => {
    ...
    try {
        ...
        // Find lab owner or staff based on identifier
        const labOwner = contactNumber
            ? await User.findOne({ contactNumber, role: "lab" })
            : await User.findOne({ email, role: "lab" });



        if (!labOwner) {
            return res.status(200).json({
                message: "Lab not exist",
                success: false
            });
        }
        ...
        const isTestMode = contactNumber == "8290818632" || contactNumber == "9917141332" || email == "test@example.com" || labOwner?.created_by == "admin";
        ...
        if (type === "forgot-password" && isValid) {
            const user = labOwner || await User.findById(labStaff?.labId);
            if (!user) {
                return res.status(200).json({
            ...
            const token = jwt.sign(
                { user: user._id, type: "lab", isOwner: !!labOwner },
        ...
        const userData = await User.findById(labOwner._id);
        if (!userData) {
            return res.status(200).json({
        ...
        const [
            ...
            LabImage.findOne({ userId: userData._id }),
            LabAddress.findOne({ userId: userData._id }),
            LabPerson.findOne({ userId: userData._id }),
            LabLicense.findOne({ userId: userData._id }),
        ]);
        ...
        const user = await User.findById(labOwner._id);
...
const forgotPassword = async (req, res) => {
    ...
    try {
        const isExist = await User.findOne({ contactNumber, role: "lab" });
        if (!isExist) {
...
const resetPassword = async (req, res) => {
    ...
    try {
        ...
        if (!isExist) return res.status(200).json({ message: 'Invalid user' });
        const hashedPassword = await bcrypt.hash(password, 10);
...
const getProfile = async (req, res) => {
    ...
    try {
        ...
        const [
            ...
            LabImage.findOne({ userId: id }),
            LabAddress.findOne({ userId: id }),
            LabPerson.findOne({ userId: id }),
            LabLicense.findOne({ userId: id }),
        ]);
...
const getProfileDetail = async (req, res) => {
    ...
    try {
        ...
        const labData = await Laboratory.findById(user.labId).select("-password").populate('category');
...
const labLicense = async (req, res) => {
    try {
        const { labLicenseNumber } = req.body;
        const userId = req.user.userId
