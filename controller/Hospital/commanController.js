...
export const doctorAppointmentInvoice = async (req, res) => {
  ...
  try {
    ...
    if (aptData?.hospitalId) {
      ...
      const hospital = await User.findById(aptData?.hospitalId)
      // let address = await HospitalAddress.findOne({ hospitalId: hospital?.hospitalId }).populate('country state city', 'name')
      const doctorAbout = await DoctorAbout.findOne({ userId: aptData?.doctorId }).select('specialty countryId stateId cityId').populate('specialty countryId stateId cityId', 'name')
      const addressParts = [
        doctorAbout?.fullAddress,
        doctorAbout?.cityId?.name,
        doctorAbout?.stateId?.name,
        doctorAbout?.countryId?.name,
        doctorAbout?.pinCode
      ];
      const dataToSend = {
        fees: aptData.fees,
...
export const opdInvoicePdf = async (req, res) => {
  const id = req.params.id
  try {
    const aptData = await DoctorAppointment.findById(id).populate('doctorId patientId hospitalId', 'name email contactNumber nh12')
    if (!aptData) {
      return res.status(404).json({ message: "Appointment data not found", success: false })
    }
    const paymentData = await DoctorAptPayment.findOne({ appointmentId: id })
    const patientDemo = await PatientDemographic.findOne({ userId: aptData?.patientId?._id }).select('dob bloodGroup age gender address').lean()
    const ptBasic = await Patient.findOne({ userId: aptData?.patientId?._id })
    const patientData = {
      ...patientDemo, name: aptData?.patientId?.name, email: aptData?.patientId?.email, nh12: aptData?.patientId?.nh12,
    ...
    const hospital = await User.findById(aptData?.hospitalId)
    const hospitaldata = await HospitalBasic.findOne({ userId: aptData?.hospitalId })
    let address = await HospitalAddress.findOne({ hospitalId: hospital?.hospitalId }).populate('country state city', 'name')
    const doctorAbout = await DoctorAbout.findOne({ userId: aptData?.doctorId }).select('specialty').populate('specialty', 'name')
    const dataToSend = {
      fees: aptData.fees,
      logoFileId: hospitaldata?.logoFileId,
      paymentMethod: paymentData?.paymentMethod,
      transactionId: paymentData?.customId,
      ...
      bookingId: aptData?.customId,
      status: aptData?.status,
      vitals: aptData?.vitals,
      customId: aptData.customId,
      date: aptData.date,
      paymentStatus: aptData.paymentStatus
    }
...
export const doctorEmpanelmentPdf = async (req, res) => {
  ...
  try {
    const isHospital = await User.findOne({ _id: hospitalId, role: "hospital" }).populate('hospitalId', 'logoFileId')
    if (!isHospital) {
    ...
    const doctorEmp = await StaffEmployement.findOne({ organizationId: hospitalId, userId: doctorId })
    if (!doctorEmp) {
      return res.status(404).json({ message: "Doctor not found", success: false })
    }


    let address = await HospitalAddress.findOne({ hospitalId: isHospital.hospitalId?._id }).populate('country state city', 'name')
    ...
    const dataToSend = {
      contractStart: doctorEmp.contractStart,
      contractEnd: doctorEmp.contractEnd,
      doctorName: isDoctor?.name,
      doctorRole: doctorEmp?.role,
      doctorNh12: isDoctor?.nh12,
      doctorSpecialty: doctorAbout?.specialty?.name,
      orgName: isHospital?.name,
      orgNh12: isHospital?.nh12,
      orgAddress: addressParts.filter(Boolean).join(', '),
      orgEmail: isHospital?.email,
      orgLogo: `api/file/${isHospital?.hospitalId?.logoFileId}`,
      orgContactNumber: isHospital?.contactNumber,
    }
...
export const pharmacyInvoicePdf = async (req, res) => {
  const id = req.params.id
  try {
    const sellData = await Sell.findOne({
      $or: [{ customId: id }, { _id: id }]
    }).populate('patientId pharId hospitalId doctorId', 'name nh12 email contactNumber')
      .populate('prescriptionId', 'customId')
      .populate('products.inventoryId', 'medicineName batchNumber')
    const ptPersonal = await Patient.findOne({ userId: sellData.patientId?._id }).lean()
    const ptDemo = await PatientDemographic.findOne({ userId: sellData.patientId?._id }).populate('cityId stateId', 'name').lean()
    const patientData = { ...ptPersonal, ...ptDemo }
    let orgData = {}
    if (sellData.hospitalId) {
      const hospitalUser = await User.findById(sellData.hospitalId?._id)
      const [hospitalbasic, hospitalPerson, hospitalAddress] = await Promise.all([
        HospitalBasic.findById(hospitalUser?.hospitalId),
        HospitalContact.findOne({ hospitalId: hospitalUser?.hospitalId }),
        HospitalAddress.findOne({ hospitalId: hospitalUser?.hospitalId }).populate('city state country', 'name'),
      ])
      const addressParts = [
      ...
      orgData.name = sellData.hospitalId.name
      orgData.nh12 = sellData.hospitalId.nh12
      orgData.email = sellData.hospitalId.email
      orgData.contactNumber = sellData.hospitalId.contactNumber
      orgData.logo = `api/file/${hospitalbasic?.logoFileId}`
      orgData.contactPerson = hospitalPerson?.name
      orgData.address = addressParts.filter(Boolean).join(', ');

    } else {
      const [pharbasic, pharPerson, pharAddress] = await Promise.all([
        Pharmacy.findOne({ userId: sellData.pharId?._id }),
        PharPerson.findOne({ userId: sellData.pharId?._id }),
        PharAddress.findOne({ userId: sellData.pharId?._id })
      ])
      const addressParts = [
      ...
      orgData.name = sellData.pharId.name
      orgData.nh12 = sellData.pharId.nh12
      orgData.email = sellData.pharId.email
      orgData.contactNumber = sellData.pharId.contactNumber
      orgData.logo = pharbasic?.logo
      orgData.contactPerson = pharPerson?.name
      orgData.address = addressParts.filter(Boolean).join(', ');
    }
...
export const pharmacyReturnPdf = async (req, res) => {
  ...
  try {
    const sellData = await Sell.findOne({
      ...
      .populate('prescriptionId', 'customId')
      .populate('products.inventoryId', 'medicineName batchNumber')
      .populate('returnProducts.inventoryId', 'medicineName batchNumber')
    const ptPersonal = await Patient.findOne({ userId: sellData.patientId?._id }).lean()
    const ptDemo = await PatientDemographic.findOne({ userId: sellData.patientId?._id }).populate('cityId stateId', 'name').lean()
    const patientData = { ...ptPersonal, ...ptDemo }
    let orgData = {}
...
export const consentLetterPdf = async (req, res) => {
  const id = req.params.id
  try {
    const isDept = await PatientDepartment.findById(id)
      .populate({ path: 'allotmentId', select: 'customId primaryDoctorId', populate: [{ path: "primaryDoctorId", select: 'name email nh12 contactNumber' }] })
      .populate('patientId hospitalId', 'name email nh12 contactNumber')
    if (!isDept) {
      return res.status(404).json({ message: "Patient data not found in this hospital", success: false })
    }
    const patientId = isDept.patientId?._id
    const hospitalId = isDept.hospitalId
    const ptBasic = await Patient.findOne({ userId: patientId }).lean()
    const ptDemo = await PatientDemographic.findOne({ userId: patientId }).populate('cityId stateId countryId').lean()
    const patientData = {
      ...ptDemo, name: isDept?.patientId?.name, email: isDept?.patientId?.email, nh12: isDept?.patientId?.nh12,
      contactNumber: isDept?.patientId?.contactNumber, gender: ptBasic?.gender
    }

    const hospital = await User.findById(hospitalId)
    const hospitalBasic = await HospitalBasic.findById(hospital?.hospitalId)
    const address = await HospitalAddress.findOne({ hospitalId: hospital?.hospitalId }).populate('country state city', 'name')
    const addressParts = [
      address?.fullAddress,
      address?.city?.name,
      address?.state?.name,
    ...
    const dataToSend = {
      ...
      logo: hospitalBasic?.logoFileId,
      name: isDept?.hospitalId?.name,
      email: isDept?.hospitalId?.email,
      nh12: isDept?.hospitalId?.nh12,
      contactNumber: isDept?.hospitalId?.contactNumber,

    }

    return res.status(200).json({ success: true, consent: isDept, ptData: patientData, hospitalData: dataToSend })
...
export const bedInvoicePdf = async (req, res) => {
  ...
  try {
    ...
    if (!allotmentData) {
      return res.status(404).json({ message: "Allotment data not found", success: false })
    }
    const dischargeData = await DischargePatient.findOne({ allotmentId: allotmentData?._id })
    const hospitalbasic = await HospitalBasic.findOne({ userId: allotmentData.hospitalId })
    const paymentData = await HospitalPayment.findOne({ allotmentId: allotmentData?._id })
    const ptPersonal = await User.findById(allotmentData?.patientId)
      .select('patientId nh12 name email contactNumber').populate('patientId', 'gender').lean()
    const ptDemo = await PatientDemographic.findOne({ userId: allotmentData?.patientId })
      .populate('cityId stateId countryId', 'name').lean()
    const hospital = await User.findById(allotmentData?.hospitalId).select('name email contactNumber nh12 hospitalId')
      .populate('hospitalId', 'logoFileId').lean()
    const hospitalAddress = await HospitalAddress.findOne({ hospitalId: hospital?.hospitalId }).populate('city state', 'name').lean()
    const patientData = { ...ptPersonal, ...ptDemo }
    const hospitalData = { ...hospital, ...hospitalAddress, logoFileId: hospitalbasic?.logoFileId }
...
export const labReportPdf = async (req, res) => {
  ...
  try {
    ...
    let labData = {}
    if (user.role == "lab") {
      const laboratory = await User.findById(user._id).populate('labId', 'logo').lean()
      const labAddress = await LabAddress.findOne({ userId: user._id }).populate('cityId stateId countryId', 'name').lean()
      labData.name = laboratory.name
      labData.email = laboratory.email
      labData.nh12 = laboratory.nh12
      labData.contactNumber = laboratory.contactNumber
      labData.logo = laboratory?.labId?.logo
      labData.address = labAddress.fullAddress
      labData.city = labAddress?.cityId
      ...
      const hospitalAddress = await HospitalAddress.findOne({ hospitalId: hospital?.hospitalId }).populate('city state country', 'name').lean()
      labData.name = hospital.name
      labData.email = hospital.email
      labData.nh12 = hospital.nh12
      labData.contactNumber = hospital.contactNumber
      labData.logo = `api/file/${hospital?.hospitalId?.logoFileId}`
      labData.address = hospitalAddress?.fullAddress
...
export const labInvoicePdf = async (req, res) => {
  ...
  try {
    const appointmentData = await LabAppointment.findOne({ $or: [{ customId: id }, { _id: id }] })
      .populate('tests.category', 'name').populate('tests.subCat.subCatId', 'subCategory code')
      .populate('patientId', 'name email contactNumber nh12').populate('staff', 'name nh12')
    if (!appointmentData) {
      return res.status(404).json({ message: "Appointment data not found", success: false })
    }
    const labPayment = await LabPayment.findOne({ appointmentId: appointmentData?._id })
    ...
    if (user.role == "lab") {
      ...
      return res.status(200).json({ message: "Report data fetched", patientData, labPayment, appointmentData, labData, success: true })
...
export const labSamplePdf = async (req, res) => {
  ...
  try {
    ...
    const labSamples = await LabSample.find({ appointmentId: appointmentData?._id }).populate('forTestId')
    ...
    if (user.role == "lab") {
      ...
      return res.status(200).json({ message: "Report data fetched", patientData, labSamples, appointmentData, labData, success: true })
...
export const labOrderPdf = async (req, res) => {
  ...
  try {
    ...
    if (isUser.role == "lab") {
      ...
      const address = await LabAddress.findOne({ userId: aptData?.labId?._id }).populate('countryId stateId cityId', 'name')
      const addressParts = [
        address?.fullAddress,
        address?.cityId?.name,
      ...
      return res.status(200).json({ success: true, orgData, ptData: patientData, orderData: { ...aptData, ...paymentData } })
    } else {
      const hospitaldata = await HospitalBasic.findOne({ userId: aptData?.labId?._id })
      const address = await HospitalAddress.findOne({ hospitalId: isUser.hospitalId }).populate('country state city', 'name')
      const addressParts = [
        address?.fullAddress,
        address?.city?.name,
        address?.state?.name,
...
export const dischargeSummaryPdf = async (req, res) => {
  ...
  try {
    const allotmentData = await BedAllotment.findOne({ $or: [{ customId: customId }, { _id: customId }] })
      .populate('primaryDoctorId', 'nh12 name')
      .populate('departmentId', 'departmentName').populate({ path: 'prescriptionId', populate: [{ path: 'doctorId', select: 'nh12' }] })
      .populate({ path: 'bedId', populate: [{ path: "roomId" }] })

    if (!allotmentData) {
      return res.status(404).json({ message: "Allotment data not found", success: false })
    }
    const dischargeData = await DischargePatient.findOne({ allotmentId: allotmentData?._id }).populate('nurseSignature doctorSignature', 'name email nh12 contactNumber')
    const hospitalbasic = await HospitalBasic.findOne({ userId: allotmentData.hospitalId })
    const paymentData = await HospitalPayment.findOne({ allotmentId: allotmentData?._id })
    const ptPersonal = await User.findById(allotmentData?.patientId)
      .select('patientId nh12 name email contactNumber').populate('patientId', 'gender').lean()
    ...
    const doctorAbout = await DoctorAbout.findOne({ userId: dischargeData?.doctorSignature?._id }).select('specialty').populate('specialty', 'name')
    const staffEmployement = await StaffEmployement.findOne({ organizationId: allotmentData?.hospitalId?._id, userId: dischargeData?.doctorSignature?._id })


    const hospitalData = { ...hospital, doctorSpecialty: doctorAbout?.specialty?.name, doctorRole: staffEmployement?.role, address: addressParts.filter(Boolean).join(', '), logoFileId: hospitalbasic?.logoFileId }
    return res.status(200).json({ message: "Data Fetched", patientData, allotmentData, hospitalData, dischargeData, paymentData, success: true })
  } catch (error) {
    console.log(error)
    return res.status(500).json({ message: error?.message, success: false })
