...
export const getHospitalDoctorEmployement = async (req, res) => {
  try {
    ...
    const employmentDetails = await StaffEmployement.findOne({ userId: user._id, organizationId: hospital?._id })?.populate('department').populate('organizationId', 'name')
...
export const availableDoctor = async (req, res) => {
  const { time, department } = req.params;
  const hospitalId = req.user.id;
  ...
  try {
    ...
    if (doctorEmp.length === 0) {
      return res.status(404).json({
        message: "No doctor available in this department",
        success: false
      });
    }
    ...
    // 3. Get timeslots for today
    const doctorTimeSlots = await TimeSlot.find({
      userId: { $in: doctorIds },
      hospitalId,
      day: today
    });

    // 4. Filter doctors who have this time slot
    const availableSlotDoctors = doctorTimeSlots.filter(doc => {
      return doc.slots.some(slot => {
        return time >= slot.startTime && time < slot.endTime;
      });
    });

    const availableDoctorIds = availableSlotDoctors.map(d => d.userId.toString());

    // 5. Get appointments for that time
    const todayAppointments = await DoctorAppointment.find({
    ...
    // 6. Final available doctors (exclude booked)
