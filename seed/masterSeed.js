...
async function seed() {
  ...
  const hosBasic = await findOrCreate(HospBasic,
    { hospitalName: "NeoHealthCard City Hospital" },
  ...
  const hosUser = await findOrCreate(User,
    ...
    {
      name: "NeoHealthCard City Hospital", contactNumber: "9100000001",
  ...
  const labProfile = await findOrCreate(Lab,
    ...
    {
      name: "NeoHealthCard Diagnostics", email: "lab@neohealth.com",
  ...
  const pharProfile = await findOrCreate(Phar,
    ...
    {
      name: "NeoHealthCard MedStore", email: "pharmacy@neohealth.com",
